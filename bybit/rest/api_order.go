...
//CreateOrderFutures CreateOrderFutures
func (b *ByBit) CreateOrderFutures(side string, orderType string, price float64,
	qty float64, timeInForce string, takeProfit float64, stopLoss float64, reduceOnly bool, tpTrigger string,
	slTrigger string, closeOnTrigger bool, orderLinkID string, symbol string) (result CreateOrderFutures, rateLimit int, err error) {
	var cResult CreateOrderFuturesResult
	...
	if tpTrigger != "" {
		params["tp_trigger_by"] = stopLoss
	}
	if slTrigger != "" {
		params["sl_trigger_by"] = true
	}
	...
	rateLimit = cResult.RateLimitStatus
	return
}
...
func (b *ByBit) CreateOrderV2(side string, orderType string, price float64,
	...
	closeOnTrigger bool, orderLinkID string, symbol string) (result OrderV2, rateLimit int, err error) {
...
func (b *ByBit) GetOrdersFutures(sort string, order string, page int,
	limit int, orderStatus string, symbol string) (result []OrderFutures, err error) {
	return b.getOrdersFutures("", "", sort, order, page, limit, orderStatus, symbol)
}
...
// page: 页码，默认取第一页数据
// limit: 一页数量，一页默认展示20条数据
func (b *ByBit) getOrdersFutures(orderID string, orderLinkID string, sort string, order string, page int,
	limit int, orderStatus string, symbol string) (result []OrderFutures, err error) {
	var cResult OrderListResultFutures
	...
	params["symbol"] = symbol
	var resp []byte
	resp, err = b.SignedRequest(http.MethodGet, "private/linear/order/list", params, &cResult) //remade on V2 /v2/private/order/list   last open-api/order/list
	if err != nil {
		return
	}
	if cResult.RetCode != 0 {
...
func (b *ByBit) getOrders(orderID string, orderLinkID string, sort string, order string, page int,
	...
	var resp []byte
	resp, err = b.SignedRequest(http.MethodGet, "/v2/private/order/list", params, &cResult) //remade on V2 /v2/private/order/list   last open-api/order/list
	if err != nil {
		return
	}
	if cResult.RetCode != 0 {
...
//CancelOrderFutures 撤销活动委托单
// orderID: 活动委托单ID, 数据来自创建活动委托单返回的订单唯一ID
// symbol:
func (b *ByBit) CancelOrderFutures(orderID string, orderLinkID string, symbol string) (result OrderCancelFutures, err error) {
	var cResult CancelOrderFuturesResult
	params := map[string]interface{}{}
	params["symbol"] = symbol
	if orderID != "" {
		params["order_id"] = orderID
...
// CancelAllOrderFutures Cancel All Active Orders
func (b *ByBit) CancelAllOrderFutures(symbol string) (result []OrderV2, err error) {
	var cResult CancelAllOrderV2Result
	params := map[string]interface{}{}
	params["symbol"] = symbol
	var resp []byte
	resp, err = b.SignedRequest(http.MethodPost, "/private/linear/order/cancel-all", params, &cResult)
	if err != nil {
		return
	}
	if cResult.RetCode != 0 {
