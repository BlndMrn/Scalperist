...
func main() {
	...
	//main ETH
	account.key, account.skey, account.exchange = "key", "secret key", "Bybit"
	account.symbols = "ETHUSD"
	mainLogic(account.symbols, account.key, account.skey, account.exchange)
...
func mainLogic(symbol string, key string, skey string, exchange string) {
	...
	for {
		...
		if position == true {
			...
			if orders == true {
				...
				switch exchange {
				case "Bybit":
					positionEntered = BybitCreateOrder(symbol, key, skey, baseURL)
					//case "Bitmex":
					//case "Deribit":
				}
...
//BybitCreateOrder open short/long positions
func BybitCreateOrder(symbol string, key string, skey string, baseURL string) (positionEntered bool) {
	...
	if ((open-close)/open)*100 > 0.1 { //if open less than close on 0.10% start Long trade
	...
	if ((open-close)/open)*100 < -0.1 { // if open more than close on 0.10% start Short trade
